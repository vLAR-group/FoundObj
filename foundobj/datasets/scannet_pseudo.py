...
class VoxelizedDataset(Dataset):
    def __init__(self, mode, cfg, batch_size):
        self.mode = mode
        self.cfg = cfg
        ...
        for scene_name in scene_list:
            if self.mode == 'train':
                ...
                if np.any(np.isin(semantic, TRAIN_OBJECT_CLASS_IDS)):
    ...
    def __getitem__(self, idx):
        ...
        if self.cfg.sp_dir is not None and self.mode == 'train':
            scene_id = os.path.splitext(os.path.basename(path))[0]
            sp_file = os.path.join(self.cfg.sp_dir, 'scene' + scene_id + '_sp.npy')
            if os.path.exists(sp_file):
                sp_idx = np.load(sp_file)
        sp_idx = sp_idx.astype(np.int32)

        if self.mode == 'train' and self.cfg.pseudo_dir is not None:
            scene_id = os.path.splitext(os.path.basename(path))[0]
            pseudo_file = os.path.join(self.cfg.pseudo_dir, 'scene' + scene_id + '.pth')
            if os.path.exists(pseudo_file):
                pseudo = torch.load(pseudo_file, map_location='cpu')
                mask = pseudo['mask'].numpy()
                instance = -np.ones(len(pc), dtype=np.int32)
                assigned = mask.any(axis=1)
                if assigned.any():
                    instance[assigned] = np.argmax(mask[assigned], axis=1).astype(np.int32) + 1
        ...
        coords, feature, semantic, instance, unique_map, inverse_map = self._voxelize(
    ...
    def _collate_fn(self, batch):
        ...
        for b in range(len(coords)):
            ...
            instance_batch.append(instance[b])
            ...
            if instance_ids.numel() == 0:
                ...
                target[b]['labels'] = torch.zeros(len(instance_ids), dtype=torch.long)
                target[b]['masks'] = masks
                target[b]['segment_mask'] = sp_masks
