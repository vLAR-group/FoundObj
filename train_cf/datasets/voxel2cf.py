...
class Voxel2vector(Dataset):
    ...
    def point_voxelize(self, pts, resolution=32):
        pts = pts + 0.5
        idx = np.floor(pts * resolution).astype(int)
    ...
    def get_instance(self, instance):
        ...
        grids = self.point_voxelize(pts, resolution=32)
        return torch.from_numpy(grids).float(), torch.from_numpy(pts).float(), torch.from_numpy(
            cf).float(), torch.from_numpy(mask).float()
    ...
    def collate_fn(self, batch):
        voxels, pts, cf, mask = list(zip(*batch))
        voxel_batch, pts_lenseq = [], []
        for batch_id, _ in enumerate(voxels):
            num_points = len(pts[batch_id])
            voxel_batch.append(voxels[batch_id][None, None, ...])
            pts_lenseq.append(num_points)
        return torch.cat(voxel_batch), torch.cat(pts), torch.cat(cf), pts_lenseq, torch.cat(mask)
