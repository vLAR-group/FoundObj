...
from foundobj.datasets.scannet_sup import VoxelizedDataset
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train Mask3D supervised on ScanNet (18 classes)")
    parser.add_argument("--data-dir", type=str, default="data/scannet/processed_aligns")
    parser.add_argument("--save-path", type=str, default="outputs/train_sup_scannet")
    ...
    parser.add_argument("--use-sp", default=True)
    parser.add_argument("--no-sp", dest="use_sp", action="store_false")
    return parser.parse_args()


def main(args, logger):
    mask3d = build_mask3d(args.mask3d_config)
    train_dataset = VoxelizedDataset('train', args, batch_size=args.batch_size, num_workers=args.num_workers)
    val_dataset = VoxelizedDataset('validation', args, batch_size=1, num_workers=4)
