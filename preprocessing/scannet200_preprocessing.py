...
class ScannetPreprocessing(BasePreprocessing):
    def __init__(
        self,
        data_dir: str = "/home/zihui/HDD/ScanNetv2",
        save_dir: str = "../data/scannet/processed_aligns2",
    ...
    def _description_filepath(self, filepath):
        return Path(filepath).parent / filepath.name.replace("_vh_clean_2.ply", ".txt")

    def _apply_axis_alignment(self, filepath, coords):
        description_filepath = self._description_filepath(filepath)
        with open(description_filepath) as txtfile:
            for line in txtfile:
                values = line.split()
                if values and values[0] == "axisAlignment":
                    align_mat = np.array([float(x) for x in values[2:]]).reshape([4, 4]).astype(np.float32)
                    return coords.dot(align_mat[:3, :3].T) + align_mat[:3, 3]
        return coords

    def _load_scene_type(self, description_filepath):
        with open(description_filepath) as f:
            lines = f.read().split("\n")[:-1]
        return lines[-1].split(" = ")[1]

    def _load_segments(self, filepath):
        segment_filepath = Path(filepath).parent / filepath.name.replace(".ply", ".0.010000.segs.json")
        segments = np.array(self._read_json(segment_filepath)["segIndices"])
        return segment_filepath, segments
    ...
    def process_file(self, filepath, mode):
        ...
        coords = self._apply_axis_alignment(filepath, coords)
        points = np.hstack((coords, features))

        if mode in ["train", "validation"]:
            # getting scene information
            description_filepath = self._description_filepath(filepath)
            filebase["scene_type"] = self._load_scene_type(description_filepath)
            ...
            segment_indexes_filepath, segments = self._load_segments(filepath)
            ...
            if self.vis:
                ...
                for point_idx in range(points.shape[0]):
                    if int(points[:, -2][point_idx]) in VALID_CLASS_IDS_200 and int(points[:, -2][point_idx])!=-1:
                        sem_colors[point_idx] = SCANNET_COLOR_MAP_200[int(points[:, -2][point_idx])]
        ...
        if not self.scannet200:
            ...
            for scene, wrong_label in found_wrong_labels.items():
                ...
                if not bug_file.exists():
                    logger.info(f"Skip missing bug-fix file {bug_file}")
                    continue
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Preprocess ScanNet200 data")
    parser.add_argument("--data-dir", "--data_dir", default="/home/zihui/HDD/ScanNetv2")
    parser.add_argument("--save-dir", "--save_dir", default="../data/scannet/processed_aligns2")
    parser.add_argument("--modes", nargs="+", default=["train", "validation", "test"])
    parser.add_argument("--n-jobs", "--n_jobs", type=int, default=10)
    parser.add_argument("--vis", action=argparse.BooleanOptionalAction, default=True)
    parser.add_argument("--git-repo", "--git_repo", default="ScanNet")
    parser.add_argument("--scannet200", action=argparse.BooleanOptionalAction, default=True)
