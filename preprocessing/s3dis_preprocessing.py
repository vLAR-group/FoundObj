...
class S3DISPreprocessing(BasePreprocessing):
    def __init__(
        self,
        data_dir: str = "/home/zihui/HDD/Stanford3dDataset_v1.2_Aligned_Version",
        save_dir: str = "data/s3dis/processed",
        modes: tuple = (
            "Area_1",
            "Area_2",
            "Area_3",
    ...
    def process_file(self, filepath, mode):
        ...
        scene_txt_filepath = f"{filepath}/{scene_name}.txt"
        if os.path.exists(scene_txt_filepath):
            pcd_size = self._buf_count_newlines_gen(scene_txt_filepath)
            if points.shape[0] != pcd_size:
                print(f"FILE SIZE DOES NOT MATCH FOR {scene_txt_filepath}")
                print(f"({points.shape[0]} vs. {pcd_size})")
        else:
            logger.warning(f"Skip missing room-level point file: {scene_txt_filepath}")
    ...
    def compute_color_mean_std(self, train_database_path: str = ""):
        ...
        color_mean = np.array(all_mean).mean(axis=0)
        color_std = np.sqrt(np.array(all_std).mean(axis=0) - color_mean**2)
        feats_mean_std = {
            "mean": [float(each) for each in color_mean],
            "std": [float(each) for each in color_std],
        }
        self._save_yaml(
            self.save_dir / "color_mean_std.yaml", feats_mean_std
        )
    ...
    def joint_database(self, train_modes=None):
        if train_modes is None:
            train_modes = self.modes
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Preprocess S3DIS data")
    parser.add_argument("--data-dir", "--data_dir", default="/home/zihui/HDD/Stanford3dDataset_v1.2_Aligned_Version")
    parser.add_argument("--save-dir", "--save_dir", default="data/s3dis/processed")
    parser.add_argument(
        "--modes",
        nargs="+",
