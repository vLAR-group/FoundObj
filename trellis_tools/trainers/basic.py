...
class BasicTrainer(Trainer):
    ...
    def init_models_and_more(self, **kwargs):
        ...
        if self.fp16_mode == 'amp':
            ...
            self.scaler = torch.GradScaler(init_scale=2**20) if self.fp16_mode == 'amp' else None
        ...
        if self.is_master:
        self.ema_params = [copy.deepcopy(self.master_params) for _ in self.ema_rate]#ema_params torch.Size([129159576])
        ...
        ##########################################
        # if self.is_master:
        #     self._init_encoder_from_ckpt(**kwargs)

        # Initialize 监督信号的encoder (固定权重)
        self._init_supervise_encoder() 

#######################################     
    def _init_encoder_from_ckpt(self, **kwargs):
        """
        Initialize encoder from a separate checkpoint, if specified.
        This is used only at first-time training (not resuming).
        """
        
        ckpt_path = kwargs.get("encoder_ckpt", None)
        if ckpt_path is None:
            print("[DEBUG] No encoder ckpt specified in config.")
            return

        if not os.path.exists(ckpt_path):
            print(f"[DEBUG] Encoder checkpoint path does not exist: {ckpt_path}")
            return
        # 选择加载方式：.safetensors or .pt
        if ckpt_path.endswith(".safetensors"):
            state_dict = load_file(ckpt_path)
        else:
            state_dict = torch.load(ckpt_path, map_location="cpu")

        print(f"[DEBUG] Loading encoder weights from {ckpt_path} ...")
        ...
        print(f"[DEBUG] Encoder loaded with {len(missing)} missing keys, {len(unexpected)} unexpected keys")
    
    def _init_supervise_encoder(self):
        # 深拷贝encoder的结构
        self.supervise_encoder = copy.deepcopy(self.models['encoder'])
        
        # 移动到正确的设备
        self.supervise_encoder = self.supervise_encoder.to(self.device)
        # 加载固定权重
        supervise_ckpt_path = "/home/zihui/SSD/TRELLIS_zhixuan/ABO_vae/vae/ss_vae_conv3d_16l8_fp16/ckpts/encoder_ema0.9999_step0060000.pt"
        supervise_state_dict = torch.load(supervise_ckpt_path, map_location=self.device, weights_only=True)
        self.supervise_encoder.load_state_dict(supervise_state_dict, strict=False)

        for param in self.supervise_encoder.parameters():
        param.requires_grad = False
        # 设置为评估模式
        self.supervise_encoder.eval()
        
        if self.is_master:
            print("[INFO] Supervise_encoder initialized with fixed weights.")
    ...
    def update_ema(self):
        ...
        for master_param, ema_param in zip(self.master_params, self.ema_params[i]):
                ema_param.detach().mul_(ema_rate).add_(master_param, alpha=1.0 - ema_rate)#ema = ema_rate * ema + (1 - ema_rate) * master_param
    ...
    def run_step(self, data_list):
        ...
        for i, mb_data in enumerate(data_list):
            ...
            ###############################
            with torch.no_grad():
                grids = mb_data['x_0'].float().to(self.device) 
                z = self.supervise_encoder(grids, sample_posterior=False)#[B, 1, 64, 64, 64]
                mb_data['x_0'] = z
            ...
            with nested_contexts(*sync_contexts), elastic_controller_context():
                with amp_context():
                    loss, status = self.training_losses(**mb_data)#mb_data
                    print(loss['mse'].item())# : x_0 torch.Size([8, 8, 16, 16, 16]), cond torch.Size([8, 3, 518, 518])
