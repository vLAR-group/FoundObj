...
class Trainer:
    ...
    def __init__(self,
        ...
        **kwargs
        ...
        #####################################################
        # 初始化 PointConditionedMixin 中的 encoder/decoder
        # for m in self.models.values():
        #     if hasattr(m, '_init_point_cond_model'):
        #         m._init_point_cond_model()
        ...
        while self.step < self.max_steps:
            ...
            if self.is_master:
                ...
                if self.step % self.i_log == 0:
                    ...
                    log_str = '\n'.join([
                        #############################
                        #f'{step}: {json.dumps(log)}' 
                        f'{step}: {json.dumps(log, default=lambda o: o.item() if hasattr(o, "item") else float(o))}'
                        for step, log in log])
    ...
    def profile(self, wait=2, warmup=3, active=5):
        ...
        """
        with torch.profiler.profile(schedule=torch.profiler.schedule(wait=wait, warmup=warmup, active=active, repeat=1),
