...
from Point_condition.point_condition import PointConditionedMixin
...
class FlowMatchingTrainer(BasicTrainer):
    ...
    def __init__(
        self,
        *args,
        t_schedule: dict = {'name': 'logitNormal', 'args': {'mean': 0.0, 'std': 1.0}},
        ...
        **kwargs
        ...
        ####################################
        # 从 kwargs 中取出你需要的配置参数（来自配置文件）
        # self.point_encoder_ckpt = kwargs.pop("point_encoder_ckpt", "/default/encoder.pt")
        # self.point_decoder_ckpt = kwargs.pop("point_decoder_ckpt", "/default/decoder.pt")
    ...
    def diffuse(self, x_0: torch.Tensor, t: torch.Tensor, noise: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        t = t.view(-1, *[1 for _ in range(len(x_0.shape) - 1)])#torch.Size([8, 1, 1, 1, 1])
        x_t = (1 - t) * x_0 + (self.sigma_min + (1 - self.sigma_min) * t) * noise#torch.Size([8, 8, 16, 16, 16])
    ...
    def sample_t(self, batch_size: int) -> torch.Tensor:
        ...
        if self.t_schedule['name'] == 'uniform':
            t = torch.rand(batch_size)#8
        elif self.t_schedule['name'] == 'logitNormal':
            mean = self.t_schedule['args']['mean']#1
            std = self.t_schedule['args']['std']#1
            t = torch.sigmoid(torch.randn(batch_size) * std + mean)
        else:
            raise ValueError(f"Unknown t_schedule: {self.t_schedule['name']}")
        return t #tensor([0.7120, 0.8035, 0.8639, 0.5879, 0.5908, 0.4921, 0.8540, 0.2697])
    ...
    def training_losses(
        ...
        **kwargs
        ...
        """
        noise = torch.randn_like(x_0)#torch.Size([8, 8, 16, 16, 16])
        t = self.sample_t(x_0.shape[0]).to(x_0.device).float()
        x_t = self.diffuse(x_0, t, noise=noise)#torch.Size([8, 8, 16, 16, 16])
        cond = self.get_cond(cond, **kwargs)#torch.Size([16, N, 3])
        
        #########################################
        # cond = self.training_models['point_encoder'](cond, sample_posterior=False)       
        
        pred = self.training_models['denoiser'](x_t, t * 1000, cond, **kwargs)#torch.Size([8, 8, 16, 16, 16]) sparse_structure_flow.py
        ...
        mse_per_instance = np.array([F.mse_loss(pred[i], target[i]).item() for i in range(x_0.shape[0])])#8
        time_bin = np.digitize(t.cpu().numpy(), np.linspace(0, 1, 11)) - 1#array([9, 8, 5, 6, 8, 6, 4, 8])
        for i in range(10):
            if (time_bin == i).sum() != 0:
                terms[f"bin_{i}"] = {"mse": mse_per_instance[time_bin == i].mean()}
#当前 batch 中，落在第 i 个时间区间的样本的 MSE 平均值，例如 bin_3 表示 t ∈ [0.3, 0.4) 的样本误差均值
#bin_i 缺失表示该 batch 中 没有样本落入该时间区间
#如果某些 bin_i 的 MSE 一直偏大或不收敛，说明该区间训练不足，可能需要增强采样或损失权重调整
        return terms, {}
...
class PointConditionedFlowMatchingCFGTrainer(PointConditionedMixin, FlowMatchingCFGTrainer):

    # def __init__(self, *args, **kwargs):
    #     # 确保 point_encoder_ckpt 等参数存在于 kwargs 中
    #     super().__init__(*args, **kwargs)
    
    # def init_models_and_more(self, **kwargs):
    #     self._init_point_cond_model()
