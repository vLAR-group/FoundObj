...
class HungarianMatcher(nn.Module):
    ...
    def memory_efficient_forward(self, outputs, targets, valid_bs):
        ...
        for bs_index, bs in enumerate(valid_bs):
            ...
            tgt_ids = torch.zeros(targets[bs_index].shape[-1], dtype=torch.long, device=out_prob.device)
            cost_class = -out_prob[:, tgt_ids]

            out_mask = outputs['pred_masks'][bs].T
            tgt_mask = targets[bs_index].to(out_mask).T

            with autocast(enabled=False):
                out_mask = out_mask.float().to(tgt_mask.device)
                tgt_mask = tgt_mask.float().to(tgt_mask.device)
    ...
    def __repr__(self):
        return f"Matcher {self.__class__.__name__}"
